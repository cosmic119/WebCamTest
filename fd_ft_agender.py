...
def  sess_eval(sess, image, logits_g, logits_a):

    with tf.Graph().as_default() as g:
      flogits_g, flogits_a = sess.run([logits_g, logits_a], feed_dict={images:image})
...
#    print('argmax a=', np.argmax(flogits_a[0]))
    age_result = age[np.argmax(flogits_a[0])]
    print('gender=',gender_result,'age=',age_result)
    return gender_result, age_result


# graph construction for evaluation
gender=['WOMAN', 'MAN']
age=['0-9','10-19','20-29','30-39','40-49','50-59','60-69','over 70']
with tf.Graph().as_default() as g:
...
#        import eval_googleNet
    ...
    def check_faces(no_faces, faces, bbox, frame):
        for i in range(no_faces):
            ...
            gender, age = sess_eval(sess, image, logits_g, logits_a)
...
#                break
            ...
            age_result = age_list.pop()
            print('age_result=',age_result)
            if gender_result == 'MAN':
                gender_count[1] = gender_count[1]+1
            else:
                gender_count[0] = gender_count[0]+1

            cv2.putText(frame, "face" + str(i) + ", result: " + gender_result + ', ' + age_result, (int(bbox[0]), int(bbox[1])),
        ...
        cv2.putText(frame, '[WOMEN, MEN]: ' + str(gender_count), (10, 430), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
